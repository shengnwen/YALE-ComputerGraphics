...
/* NewType reflectance model (per-pixel) - oren
 /**************************************************************/

var PerPixelNewType_vs = "\
precision highp float;                                               \n\
                                                                     \n\
uniform mat4 uModelViewProjectionMatrix;                             \n\
uniform mat4 uModelViewMatrix;                                       \n\
...
\
  // diffuse component                                               \n\
  vec3 diffuse_2 = uLightColor * NdotL;                                \n\
\
  // D term (gaussian)                                               \n\
  float alpha_2 = acos(NdotH);                                         \n\
  float D = uC * exp(- alpha * alpha * uC);                          \n\
  // Geometric factor (G)                                            \n\
  float G1 = 2.0 * NdotH * NdotV / VdotH;                            \n\
  float G2 = 2.0 * NdotH * NdotL / VdotH;                            \n\
...
\
  ...
  vec3 specular = uLightColor * (F * D * G) / NdotV;                \n\
  // final color                                                     \n\
  gl_FragColor = (gl_FragColor + 0.4*vec4(diffuse_2, 1.0) + 0.8*vec4(specular, 1.0)) / 2.0;   \n\
\
\n\
  \n\
}                                                                    \n\
